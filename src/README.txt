...
Useful for tools style customization. The best mate for rGuiLayout tool.

rGuiStyler comes in two flavours: ZERO (free version) and ONE (low-cost version).
...
## rGuiStyler ZERO features
 ...
 - Color palette menu for quick color save/selection
 - Save style as a plain text file (.rgs)
 - Load text or binary style file (.rgs/.rgsb)
...
## rGuiStyler ONE plus features
 ...
 - Export style as binary file (.rgsb), including font data
 - Export style as code file (.h), including font data
 
## rGuiStyler Usage

The tool is pretty intuitive and easy-to-use, just choose the control to edit, 
the property to edit and select a value for that property (color, number...). 
Changes are previewed in real time in the same tool!

Once a style has been created, just save it as a style text file (.rgs) or 
export a style image table (.png) for preview and reference.

rGuiStyler ONE also allows exporting style as a portable binary file (.rgsb) and
a embeddable code file (.h), both including font data.

rGuiStyler ONE comes with a powerful command line for style format conversion and edition. 
