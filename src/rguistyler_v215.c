...
int main(int argc, char *argv[])
{
    ...
    while (!exitWindow)             // Detect window close button
    {
        ...
        if (IsFileDropped())
        {
            ...
            {
                UnloadFont(font);
                // NOTE: Font generation size depends on spinner selection
        ...
        BeginDrawing();
            ...
            GuiListViewEx((Rectangle){ anchorMain.x + 155, anchorMain.y + 40, 180, 560 }, guiPropsText, NUM_PROPS_DEFAULT, NULL, &currentSelectedProperty, NULL, NULL, true);
